...
final class WorldRegionSource implements ChunkSource {
    ...
    WorldRegionSource(ServerLevel level, MinecraftServer server, int readerCacheSize, Consumer<String> skipLog) {
        ...
        this.biomes = this.registryAccess.lookupOrThrow(Registries.BIOME);
        this.defaultBiome = this.biomes.getOrThrow(Biomes.PLAINS);
    ...
    BlockState defaultBlockState() { return Blocks.AIR.defaultBlockState(); }
    IdMap<Holder<Biome>> biomeIds() { return biomes.asHolderIdMap(); }
    ...
    private SavedChunk readChunk(int chunkX, int chunkZ, BlockEntityPolicy blockEntities) throws IOException {
        ...
        into.begin(biomes, defaultBlockState(), defaultBiome, minSectionY, maxSectionY, blockEntities);
